...
from actions.screen_processor import screen_process, _ensure_started
...
def _load_system_prompt() -> str:
    try:
        ...
        return (
            "You are JARVIS, Muhammed Razil's AI assistant. "
...
def _update_memory_async(user_text: str, jarvis_text: str) -> None:
    ...
    try:
        ...
        raw = model.generate_content(
            f"Extract personal facts from this message.\n"
...
def _log_to_desktop(header: str, content: str) -> None:
    """
    Appends formatted JARVIS session data to a daily log file 
    inside a 'J.A.R.V.I.S' folder on the user's desktop.
    """
    try:
        # Define and create the folder
        desktop = Path.home() / "Desktop"
        jarvis_folder = desktop / "J.A.R.V.I.S"
        
        # Robust folder check
        if jarvis_folder.exists() and not jarvis_folder.is_dir():
            jarvis_folder.unlink() # Delete file if it's blocking the folder
            
        jarvis_folder.mkdir(parents=True, exist_ok=True)
        
        # Define the daily log file inside the folder
        date_str = datetime.now().strftime("%Y-%m-%d")
        log_file = jarvis_folder / f"Daily_Log_{date_str}.txt"
        ...
        print(f"[Log] ⚠️ Failed to log to desktop folder: {e}")
...
TOOL_DECLARATIONS = [
    {
        "name": "open_app",
        "description": (
            "Opens one or multiple applications or websites on the Windows computer. "
            "Use 'and' or commas for multiple apps (e.g. 'open YouTube, Instagram, and CapCut'). "
            "If an app is not installed, JARVIS will automatically open it as a web app in Brave. "
        ...
        "parameters": {
            ...
            "properties": {
                "app_name": {
                    "type": "STRING",
                    "description": "Name(s) of the application(s) or website(s) to open."
                }
            },
            "required": ["app_name"]
        }
    ...
    {
        "name": "close_app",
        "description": "Closes a specific application or browser by name.",
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "app_name": {
                    "type": "STRING",
                    "description": "Name of the application to close (e.g. 'CapCut', 'Chrome')"
                }
            },
            "required": ["app_name"]
        }
    },
{
    ...
    "description": (
        "CRITICAL: Use this for ALL single-step system controls: volume (up/down/mute/set), "
        "brightness, power (sleep, hibernate, restart, shutdown), lock screen, "
        "window management (minimize, maximize, close, snap, switch), "
        "keyboard shortcuts (copy, paste, undo, redo, save, enter, escape), "
        "typing text, dark mode, and WiFi toggle. "
        "ALSO use for repeated simple actions: 'refresh 10 times' → action: reload_n, value: 10. "
        "NEVER route these to agent_task or computer_control if this tool can do it directly."
    ),
    "parameters": {
        "type": "OBJECT",
        "properties": {
...
},
    {
        ...
        "description": (
            "Controls the web browser (Brave). Use for: opening multiple websites (link with commas), "
            "searching, clicking elements, closing specific tabs or the whole browser, "
            "filling forms, scrolling, any web-based task."
        ),
        "parameters": {
            "type": "OBJECT",
            "properties": {
                "action":      {"type": "STRING", "description": "go_to | search | click | type | scroll | fill_form | smart_click | smart_type | get_text | press | close | close_tab"},
                "url":         {"type": "STRING", "description": "URL for go_to action. Use commas for multiple URLs."},
                "query":       {"type": "STRING", "description": "Search query OR tab query for close_tab"},
                "selector":    {"type": "STRING", "description": "CSS selector for click/type"},
                "text":        {"type": "STRING", "description": "Text to click, type, or match for close_tab"},
    ...
    "name": "agent_task",
    "description": (
        "Executes complex MULTI-STEP tasks that require coordination of DIFFERENT tools. "
        "Examples: 'research X and save to file', 'find files and organize them', "
        "'fill a form on a website', 'write and test code'. "
        "CRITICAL: DO NOT use for simple system commands like volume, brightness, sleep, "
        "lock screen, restart, close window, scroll, or minimize. "
        "Those MUST be routed to computer_settings for immediate execution."
    ),
    "parameters": {
        "type": "OBJECT",
        "properties": {
...
class JarvisLive:
    ...
    def __init__(self, ui: JarvisUI):
        self.ui             = ui
        self.ui_amplitude   = 0.0 # Initialized ui_amplitude
        self.session        = None
    ...
    def send_text(self, text: str):
        ...
        print(f"[JARVIS] [TEXT] Sending text: {text}")
        if not self._loop or not self.session:
            print("[JARVIS] ⚠️ Cannot send text: Session or Loop not initialized.")
            return
        asyncio.run_coroutine_threadsafe(
            self.session.send_client_content(
                turns={"parts": [{"text": text}]},
    ...
    async def _execute_tool(self, fc) -> types.FunctionResponse:
        ...
        self.ui.write_log(f"SYS: Working on {name.replace('_', ' ')}...")
        ...
        try:
            if name == "open_app":
                ...
                r = await loop.run_in_executor(
                    None, lambda: close_app(parameters=args, response=None, player=self.ui)
                )
        ...
        # Log tool execution to desktop
        log_content = f"Tool: {name}\nArguments: {json.dumps(args, indent=2)}\n{'-'*20}\nResult:\n{result}"
        _log_to_desktop("TOOL EXECUTION", log_content)
    ...
    async def _listen_audio(self):
        mic_buffer = deque(maxlen=8) # Buffer to keep last ~0.5s of audio
        ...
        try:
            while True:
                ...
                if len(audio_data) > 0:
                    ...
                    amp_scaled = rms / 32768.0
                    self.ui.set_amplitude(amp_scaled)
                    
                    # Clap / Wake detection logic
                    # A sharp sound (clap) or voice will trigger
                    # Threshold: 0.15. Don't auto-activate if Jarvis is already speaking (prevents self-interruption)
                    if amp_scaled > 0.15 and not self.ui.speaking:
                        if not self.ui.mic_active:
                            print(f"[JARVIS] [MIC] Local sound spike (amp={amp_scaled:.3f}). Activating mic...")
                            # Flush the buffer to the server to catch the beginning of the sentence
                            while mic_buffer:
                                b_data = mic_buffer.popleft()
                                await self.out_queue.put({"data": b_data, "mime_type": "audio/pcm"})
                        self.ui.activate_mic_for(3.0)  # extend by 3 seconds for every loud burst
                
                if self.ui.mic_active:
                    await self.out_queue.put({"data": data, "mime_type": "audio/pcm"})
                else:
                    mic_buffer.append(data) # Buffer silent/background audio
        except Exception as e:
                ...
                async for response in turn:
                    ...
                    if response.server_content:
                        ...
                        if sc.input_transcription and sc.input_transcription.text:
                            ...
                            if txt:
                                in_buf.append(txt)
                                # Stream input transcription (low opacity or prefix to indicate it's current)
                                # For simplicity, we just keep it in buffer for the final 'You:' log
                                # but we could stream it too if we want 'live' subtitles.

                        if sc.output_transcription and sc.output_transcription.text:
                            txt = sc.output_transcription.text
                            if txt:
                                out_buf.append(txt)
                                # Stream AI output immediately!
                                if len(out_buf) == 1:
                                    self.ui.stream_log("Jarvis: ", "ai")
                                self.ui.stream_log(txt, "ai")
                        ...
                        if sc.turn_complete:
                            ...
                            # Log conversation to desktop
                            if full_in:
                                _log_to_desktop("USER INPUT", full_in)
                            if full_out:
                                _log_to_desktop("JARVIS RESPONSE", full_out)
                    ...
                    if response.tool_call:
                        ...
                        for fc in response.tool_call.function_calls:
                            print(f"[JARVIS] [TOOL] Tool call: {fc.name}")
    ...
    async def _play_audio(self):
        ...
        try:
            while True:
                # Use a wait for the first chunk to avoid busy waiting or premature stop_speaking calls
                ...
                if len(audio_data) > 0:
                    rms = np.sqrt(np.mean(audio_data.astype(np.float32)**2))
                    self.ui.set_amplitude(rms / 32768.0)
                
                await asyncio.to_thread(stream.write, chunk)
                
                # While the queue still has data, keep playing
                ...
                # No more data in queue immediately after, mark speaking as done
                self.ui.stop_speaking()
                self.ui.set_amplitude(0.0)
        except Exception as e:
            print(f"[JARVIS] ❌ Play error: {e}")
            traceback.print_exc() # Added traceback for consistency
            raise
        finally:
            stream.close()
        ...
        while True:
            try:
                ...
                async with (
                    ...
                    self.out_queue      = asyncio.Queue(maxsize=30) # Increased maxsize to prevent blocking
                ...
                print(f"[JARVIS] [ERROR] Error: {e}")
                traceback.print_exc()

            print("[JARVIS] [RETRY] Reconnecting in 1s...")
...
def main():
    ...
    def runner(jarvis_instance):
        ...
        # Pre-warm the vision session to prevent camera analysis delay
        threading.Thread(target=_ensure_started, kwargs={"player": ui}, daemon=True).start()
    ...
    ui.set_chat_callback(jarvis.send_text)
    
    threading.Thread(target=runner, args=(jarvis,), daemon=True).start()
    ui.root.mainloop()
