...
#   screen_debug  Screenshot al, ekrandaki kodu/hatay Gemini ile analiz et ve dzelt
...
JARVIS_DIR         = Path.home() / "Desktop" / "J.A.R.V.I.S"
DESKTOP            = JARVIS_DIR  # Kept for backward-compat, now points to J.A.R.V.I.S
...
def _resolve_save_path(output_path: str, language: str) -> Path:
    ...
    # Extension->subfolder mapping inside J.A.R.V.I.S
    subfolder_map = {
        ".py": "Python", ".pyw": "Python",
        ".js": "Code", ".ts": "Code", ".html": "Code",
        ".css": "Code", ".json": "Code", ".cpp": "Code",
        ".c": "Code", ".java": "Code",
        ".sh": "Code", ".ps1": "Code",
        ".txt": "Text", ".md": "Text", ".log": "Text",
        ".sql": "Code", ".rs": "Code", ".go": "Code",
    }
    ...
    if output_path:
        ...
        if p.is_absolute():
            return p
        # Relative path: resolve inside J.A.R.V.I.S with subfolder
        ext = p.suffix.lower()
        subfolder = subfolder_map.get(ext, "Others")
        JARVIS_DIR.mkdir(parents=True, exist_ok=True)
        (JARVIS_DIR / subfolder).mkdir(exist_ok=True)
        return JARVIS_DIR / subfolder / p
    ...
    JARVIS_DIR.mkdir(parents=True, exist_ok=True)
    (JARVIS_DIR / subfolder).mkdir(exist_ok=True)
    return JARVIS_DIR / subfolder / f"jarvis_code{ext}"
...
def _detect_intent(description: str, file_path: str, code: str) -> str:
    ...
    screen_kw = ["ekrandaki", "screen", "ekranda", "bu hatay", "why am i getting",
                 "neden hata", "what's wrong", "ne yanl", "screenshot", "grnt"]
    if any(k in desc for k in screen_kw):
        return "screen_debug"

    # Optimize keywords
    optimize_kw = ["optimize", "refactor", "clean up", "improve", "temizle",
                   "iyiletir", "daha iyi", "make it better", "hzlandr"]
    ...
    if file_path:
        ...
        edit_kw  = ["edit", "update", "modify", "change", "add", "remove",
                    "refactor", "fix", "rename", "replace", "dzenle", "deitir"]
        run_kw   = ["run", "execute", "launch", "start", "altr"]
    ...
    explain_kw = ["explain", "what does", "describe", "analyze", "akla", "ne yapyor"]
