...
#
# Kullanc "sesi a", "uygulamay kapat", "tam ekran yap", "unu yaz" gibi
# bilgisayar kontrol komutlar verdiinde bu dosya devreye girer.
#
...
def brightness_up():
    if _OS == "Windows":
        os.system("powershell -c \"(Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightnessMethods).WmiSetBrightness(1, (Get-WmiObject -Namespace root/WMI -Class WmiMonitorBrightness).CurrentBrightness + 10)\"")
    elif _OS == "Darwin":
...
def sleep_computer():
    """Puts the computer to sleep (Windows)."""
    if _OS == "Windows":
        # rundll32.exe powrprof.dll,SetSuspendState 0,1,0
        # Note: If hibernation is enabled, this might hibernate instead.
...
def hibernate_computer():
    """Hibernates the computer (Windows)."""
    if _OS == "Windows":
        os.system("rundll32.exe powrprof.dll,SetSuspendState 1,1,0")
    elif _OS == "Darwin":
        pass # macOS doesn't have a simple hibernate cmd like Windows
    else:
...
def _detect_action(description: str) -> dict:
    """
    Gemini ile kullancnn ne yapmak istediini anlar.
    Herhangi bir dilde alr.
    Dner: {"action": str, "value": optional}
    """
...
- "uygulamay kapat"  {{"action": "close_app", "value": null}}
...
- "sesi ks"  {{"action": "volume_down", "value": null}}
- "sesi a"  {{"action": "volume_up", "value": null}}
- "sustur"  {{"action": "mute", "value": null}}
- "monte le son"  {{"action": "volume_up", "value": null}}
- "ekran kapat"  {{"action": "sleep_display", "value": null}}
- "monitr kapat"  {{"action": "sleep_display", "value": null}}
- "turn off screen"  {{"action": "sleep_display", "value": null}}
- "turn off monitor"  {{"action": "sleep_display", "value": null}}
- "bilgisayar yeniden balat"  {{"action": "restart", "value": null}}
- "restart the computer"  {{"action": "restart", "value": null}}
- "bilgisayar kapat"  {{"action": "shutdown", "value": null}}
- "shut down"  {{"action": "shutdown", "value": null}}
- "ekran kilitle"  {{"action": "lock_screen", "value": null}}
- "lock the screen"  {{"action": "lock_screen", "value": null}}
- "klt"  {{"action": "minimize", "value": null}}
- "minimize the window"  {{"action": "minimize", "value": null}}
- "byt"  {{"action": "maximize", "value": null}}
- "parlakl artr"  {{"action": "brightness_up", "value": null}}
- "parlakl azalt"  {{"action": "brightness_down", "value": null}}
- "increase brightness"  {{"action": "brightness_up", "value": null}}
- "wifi'yi a"  {{"action": "toggle_wifi", "value": null}}
- "toggle wifi"  {{"action": "toggle_wifi", "value": null}}
- "masastn gster"  {{"action": "show_desktop", "value": null}}
- "show desktop"  {{"action": "show_desktop", "value": null}}
- "yeni sekme a"  {{"action": "new_tab", "value": null}}
- "sekmeyi kapat"  {{"action": "close_tab", "value": null}}
- "geri git"  {{"action": "go_back", "value": null}}
- "ileri git"  {{"action": "go_forward", "value": null}}
- "sayfay yenile"  {{"action": "refresh_page", "value": null}}
- "yaknlatr"  {{"action": "zoom_in", "value": null}}
- "uzaklatr"  {{"action": "zoom_out", "value": null}}
- "kaydet"  {{"action": "save", "value": null}}
- "geri al"  {{"action": "undo", "value": null}}
- "screenshot al"  {{"action": "screenshot", "value": null}}
- "ekran grnts al"  {{"action": "screenshot", "value": null}}
- "aa kaydr"  {{"action": "scroll_down", "value": null}}
- "yukar kaydr"  {{"action": "scroll_up", "value": null}}
- "karanlk mod"  {{"action": "dark_mode", "value": null}}
...
def computer_settings(
    ...
    """
    Bilgisayar ayarlar ve UI kontrolleri.

    parameters:
        action      : lem ad (verilmezse description'dan Gemini ile tespit edilir)
        description : Kullancnn doal dil komutu (herhangi bir dilde)
        value       : leme zg deer (ses seviyesi, yazlacak metin, tekrar says vb.)
    """
    if not _PYAUTOGUI:
