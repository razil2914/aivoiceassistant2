...
SYSTEM_PROMPT = (
    ...
    "Analyze images with technical precision and perform a comprehensive scan. "
    "Identify ALL distinct objects, people, texts, and important details in the scene. "
    ...
    "Respond in maximum 3 short sentences. Speed is priority. "
...
def _get_camera_index() -> int:
    ...
    for idx in range(6):
        ...
        if ret and frame is not None and frame.mean() > 5:
            ...
            print(f"[Camera]   Index {idx}: no valid frame (black or empty).")
...
def _capture_camera(player=None) -> bytes:
    # Use shared frame from UI if camera is already open
    if player is not None and hasattr(player, 'latest_camera_frame') and player.latest_camera_frame is not None:
        frame = player.latest_camera_frame.copy()
        if _PIL_OK:
            # frame from UI is already RGB
            img = PIL.Image.fromarray(frame)
            img.thumbnail([IMG_MAX_W, IMG_MAX_H], PIL.Image.BILINEAR)
            buf = io.BytesIO()
            img.save(buf, format="JPEG", quality=JPEG_Q, optimize=False)
            return buf.getvalue()
        _, buf = cv2.imencode(".jpg", cv2.cvtColor(frame, cv2.COLOR_RGB2BGR), [cv2.IMWRITE_JPEG_QUALITY, JPEG_Q])
        return buf.tobytes()
        ...
        while True:
            ...
            if self._session:
                ...
                try:
                    ...
                    # Re-queue the failed item so it's not lost when session restarts
                    try:
...
def screen_process(
    ...
    try:
        if angle == "camera":
            image_bytes = _capture_camera(player=player)
