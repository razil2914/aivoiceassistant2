...
def _get_first_video_url(query: str) -> str | None:
    """Scrape YouTube search results and return the URL of the first video."""
    if not _REQUESTS_OK:
        return None
    try:
        ...
        print(f"[YouTube]  Search scrape failed: {e}")
    return None
...
def _handle_play(parameters: dict, player) -> str:
    ...
    # Try to get the direct video URL first (most reliable)
    video_url = _get_first_video_url(query)

    open_browser()

    if video_url:
        print(f"[YouTube]  Navigating directly to: {video_url}")
        pyautogui.hotkey("ctrl", "l")
        time.sleep(0.3)
        pyautogui.write(video_url, interval=0.02)
        pyautogui.press("enter")
        return f"Playing YouTube video for: {query}"

    # Fallback: search page + try thumbnail click
    search_query = query.replace(" ", "+")
    ...
    if thumbnails:
        # Pick the 2nd thumbnail if available (1st is often an ad)
        x, y = thumbnails[1] if len(thumbnails) >= 2 else thumbnails[0]
        print(f"[YouTube]  Clicking thumbnail at ({x}, {y})")
        pyautogui.click(x, y)
        return f"Playing YouTube video for: {query}"

    # Last resort: keyboard navigation to first result
    print("[YouTube]  No thumbnails found, using keyboard navigation fallback")
    time.sleep(1.0)
    for _ in range(5):
        pyautogui.press("tab")
        time.sleep(0.15)
