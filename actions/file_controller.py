...
def _resolve_path(raw: str) -> Path:
    ...
    """
    default_base = Path.home() / "Desktop" / "J.A.R.V.I.S"
    
    shortcuts = {
        "desktop":   default_base,
    ...
    # If it's just a filename or a relative path, default to J.A.R.V.I.S log folder
...
def create_file(path: str, content: str = "") -> str:
    """Creates a new file with optional content, organized by type."""
    try:
        target = Path(path).expanduser()
        
        # Auto-organization logic for J.A.R.V.I.S folder
        base_jarvis = Path.home() / "Desktop" / "J.A.R.V.I.S"
        
        # If the file is being created inside the J.A.R.V.I.S folder, sort it
        if base_jarvis in target.parents or target.parent == base_jarvis:
            ...
            type_map = {
                "Text": [".txt", ".md", ".rtf", ".log"],
                "Python": [".py", ".pyw"],
                "Code": [".js", ".html", ".css", ".json", ".cpp", ".c", ".java", ".ts"],
                "Documents": [".pdf", ".docx", ".doc", ".xlsx", ".csv"],
                "Images": [".jpg", ".jpeg", ".png", ".gif", ".bmp"],
            }
            
            for f_name, extensions in type_map.items():
                if ext in extensions:
                    folder_name = f_name
                    break
            
            # Re-route to subfolder
            target = base_jarvis / folder_name / target.name

        target.parent.mkdir(parents=True, exist_ok=True)
        target.write_text(content, encoding="utf-8")
        return f"File created: {target.name} in {target.parent.name} folder."
    except Exception as e:
