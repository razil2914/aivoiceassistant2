...
def _get_brave_executable() -> str | None:
    ...
    try:
        ...
        candidate_keys = [
            ...
            r"SOFTWARE\Clients\StartMenuInternet\Brave\shell\open\command",
        ]
            ...
            for hive in [winreg.HKEY_LOCAL_MACHINE, winreg.HKEY_CURRENT_USER]:
                try:
                    ...
                    if exe and Path(exe).exists():
                        print(f"[Browser]  Brave found via registry: {exe}")
                        return exe
                except Exception:
                    continue
        ...
        for p in common_paths:
            if p.exists():
                print(f"[Browser]  Brave found via path: {p}")
                return str(p)
...
class _BrowserThread:
    ...
    async def _launch(self):
        # In JARVIS MARK XXX, we prefer Brave for better privacy and control.
        prog_id = _get_default_browser_id()
        
        # Force Brave check first
        brave_path = _get_brave_executable()
        
        if not brave_path:
            system   = platform.system()
            os_bins  = _BROWSER_BINARIES.get(system, {})
            brave_binaries = os_bins.get("brave", [])
            for b in brave_binaries:
                path = shutil.which(b)
                if path:
                    brave_path = path
                    break
        ...
        if brave_path:
            engine_name, exe_path, channel = "chromium", brave_path, None
        else:
        ...
        try:
            ...
            print(f"[Browser]  Launch failed ({e}), falling back to built-in Chromium")
    ...
    async def _launch_native_brave(self, url: str) -> bool:
        """Launches Brave natively or opens a new tab in the running instance."""
        brave_path = _get_brave_executable()
        if not brave_path:
            return False
            
        try:
            # On Windows, just running the exe with a URL will open a new tab in existing process
    ...
    async def _go_to(self, url: str) -> str:
        ...
        # Try to open in the native Brave browser first (to keep user session/cookies)
        if await self._launch_native_brave(url):
            return f"Opened in Brave: {url}"

        # Fallback to Playwright if native launch fails
        page = await self._get_page()
        try:
            await page.goto(url, wait_until="domcontentloaded", timeout=15000)
            return f"Opened (Fallback): {page.url}"
    ...
    async def _close_tab(self, query: str = None) -> str:
        """
        Closes a tab. For native Brave, we use keyboard shortcuts (Ctrl+W).
        For Playwright-managed tabs, we use the Playwright API.
        """
        import pyautogui
        if not self._context:
            # Blind attempt for native browser
            pyautogui.hotkey("ctrl", "w")
            return "Sent close command (Ctrl+W) to native browser."

        pages = self._context.pages if self._context else []
        
        # If no query, just close current
        if not query:
            if self._page:
                await self._page.close()
                self._page = self._context.pages[-1] if self._context.pages else None
                return "Playwright tab closed."
            else:
                pyautogui.hotkey("ctrl", "w")
                return "Sent close command (Ctrl+W) to browser."

        # Search Playwright pages
        for page in pages:
            try:
                url   = page.url.lower()
                title = (await page.title()).lower()
                if query.lower() in url or query.lower() in title:
                    await page.close()
                    return f"Playwright tab '{query}' closed."
            except Exception:
                continue
                
        # If not found in Playwright, assume it's native and try to close it via shortcuts
        # (This is imprecise but works for single window focus)
        pyautogui.hotkey("ctrl", "w")
        return f"Sent close command to browser for '{query}'."
...
def browser_control(
    ...
    try:
        if action == "go_to":
            url_str = parameters.get("url", "")
            if "," in url_str:
                urls = [u.strip() for u in url_str.split(",")]
                results = []
                for u in urls:
                    results.append(_bt.run(_bt._go_to(u)))
                result = " | ".join(results)
            else:
                result = _bt.run(_bt._go_to(url_str))

        elif action == "close_tab":
            result = _bt.run(_bt._close_tab(parameters.get("query") or parameters.get("text")))
        ...
        elif action == "close" or action == "close_browser":
